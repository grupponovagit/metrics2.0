@props([
    ...
    'bordered' => false, // Bordi
    'hover' => true,     // Hover effect
    'compact' => false,  // Riduce padding
])
...
<div class="overflow-x-auto rounded-xl border border-base-300 shadow-lg">
                        ...
                        <div class="flex items-center gap-2 {{ str_contains($header['class'] ?? '', 'text-right') ? 'justify-end' : '' }}">
                            ...
                            @if($header['sortable'] ?? false)
                                <div class="flex flex-col">
                                    <svg class="w-3 h-3 text-base-content/30" fill="currentColor" viewBox="0 0 20 20">
        ...
        <tbody class="bg-base-100">
            @forelse($rows as $index => $row)
                <tr class="
                    ...
                    {{ $hover ? 'hover:bg-base-200/50 transition-colors duration-150' : '' }}
                    {{ $index % 2 === 0 && $striped ? 'bg-base-50' : '' }}
                ">
                    @foreach($row as $cell)
