<x-admin.wrapper :containerless="true">
    <x-slot name="title">{{ __('Import Indeed') }}</x-slot>
    
    <x-admin.page-header 
        title="Import Indeed" 
        subtitle="Importazione candidati da Indeed"
        icon="download"
        iconColor="secondary"
    >
        <x-slot name="actions">
    ...
    <x-admin.card tone="light" shadow="lg" padding="loose">
        <div class="text-center py-12">
            <div class="w-24 h-24 bg-secondary/10 rounded-3xl flex items-center justify-center mx-auto mb-6">
                <x-admin.fa-icon name="download" class="h-16 w-16 text-secondary" />
            </div>
            <h2 class="text-2xl font-bold text-base-content mb-4">Import Indeed</h2>
            <p class="text-base-content/70 text-lg mb-6">
                Questa vista è pronta per essere personalizzata con import Indeed.
            </p>
