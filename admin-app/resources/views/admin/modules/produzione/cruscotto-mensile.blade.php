<x-admin.wrapper :containerless="true">
    <x-slot name="title">{{ __('Cruscotto Mensile') }}</x-slot>
    
    <x-admin.page-header 
        title="Cruscotto Mensile" 
        subtitle="Analisi mensile produzione"
        icon="calendar-check"
        iconColor="warning"
    >
        <x-slot name="actions">
    ...
    <x-admin.card tone="light" shadow="lg" padding="loose">
        <div class="text-center py-12">
            <div class="w-24 h-24 bg-warning/10 rounded-3xl flex items-center justify-center mx-auto mb-6">
                <x-admin.fa-icon name="calendar-check" class="h-16 w-16 text-warning" />
            </div>
            <h2 class="text-2xl font-bold text-base-content mb-4">Cruscotto Mensile</h2>
            <p class="text-base-content/70 text-lg mb-6">
                Questa vista è pronta per essere personalizzata con analisi mensile.
            </p>
