<x-admin.wrapper :containerless="true">
    <x-slot name="title">{{ __('Stato Sistema') }}</x-slot>
    
    <x-admin.page-header 
        title="Stato Sistema" 
        subtitle="Monitor stato e health check"
        icon="heartbeat"
        iconColor="success"
    >
        <x-slot name="actions">
    ...
    <x-admin.card tone="light" shadow="lg" padding="loose">
        <div class="text-center py-12">
            <div class="w-24 h-24 bg-success/10 rounded-3xl flex items-center justify-center mx-auto mb-6">
                <x-admin.fa-icon name="heartbeat" class="h-16 w-16 text-success" />
            </div>
            <h2 class="text-2xl font-bold text-base-content mb-4">Stato Sistema</h2>
            <p class="text-base-content/70 text-lg mb-6">
                Questa vista è pronta per essere personalizzata con stato sistema.
            </p>
