<x-admin.wrapper>
    <x-slot name="title">Nuova Conversione Esito Committente</x-slot>
...
<div class="min-h-screen p-6">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-6">
            <div class="text-sm breadcrumbs">
                <ul>
                    ...
                    <li>Nuova Conversione</li>
                </ul>
            </div>
            <h1 class="text-3xl font-bold mt-2">Nuova Conversione Esito</h1>
            <p class="text-sm text-base-content/70 mt-1">
                Aggiungi una nuova mappatura tra esito specifico ed esito globale
            </p>
        </div>
        ...
        <div class="card bg-base-100 shadow-xl">
            <div class="card-body">
                <form action="{{ route('admin.ict.esiti_conversione.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div class="form-control">
                        ...
                        <input type="text" name="esito_originale" class="input input-bordered" placeholder="Es: ATTIVATA, RESPINTA, IN LAVORAZIONE..." value="{{ old('esito_originale') }}" required>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            @foreach($esitiGlobali as $key => $label)
                                <label class="label cursor-pointer bg-base-200 p-4 rounded-lg hover:bg-base-300 transition">
                                    ...
                                    <input type="radio" name="esito_globale" value="{{ $key }}" class="radio radio-primary" {{ old('esito_globale') === $key ? 'checked' : '' }} required>
                                </label>
                            @endforeach
                        </div>
                        @error('esito_globale')
                    ...
                    <div class="form-control">
                        ...
                        <textarea name="note" class="textarea textarea-bordered h-24" placeholder="Note aggiuntive sulla conversione (opzionale)">{{ old('note') }}</textarea>
                        @error('note')
                            <label class="label">
                                <span class="label-text-alt text-error">{{ $message }}</span>
                            </label>
                    ...
                    <div class="alert alert-info">
                        ...
                        <div>
                            <div class="font-bold">Nota Importante</div>
                            <div class="text-xs">Non è possibile avere duplicati: la combinazione Commessa + Esito Originale deve essere unica.</div>
                        </div>
                    </div>
