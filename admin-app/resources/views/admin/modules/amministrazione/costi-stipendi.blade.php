<x-admin.wrapper :containerless="true">
    <x-slot name="title">{{ __('Costi Stipendi') }}</x-slot>
    
    <x-admin.page-header 
        title="Costi Stipendi" 
        subtitle="Gestione e monitoraggio costi stipendiali"
        icon="money-bills"
        iconColor="accent"
    >
        <x-slot name="actions">
    ...
    <x-admin.card tone="light" shadow="lg" padding="loose">
        <div class="text-center py-12">
            <div class="w-24 h-24 bg-accent/10 rounded-3xl flex items-center justify-center mx-auto mb-6">
                <x-admin.fa-icon name="money-bills" class="h-16 w-16 text-accent" />
            </div>
