<x-admin.wrapper :containerless="true">
            ...
            <div class="overflow-x-auto">
                <table class="table w-full" id="dataTable">
                    <thead class="bg-base-200">
                        <tr>
                            <th class="px-3 py-3 text-left text-xs font-semibold text-base-content uppercase tracking-tight">
                                <x-ui.icon name="calendar-week" class="h-3 w-3 inline mr-1" />
                                Sett.
                            </th>
                            <th class="px-3 py-3 text-left text-xs font-semibold text-base-content uppercase tracking-tight">
                                <x-ui.icon name="percentage" class="h-3 w-3 inline mr-1" />
                                Incr.
                            </th>
                            <th class="px-3 py-3 text-right text-xs font-semibold text-base-content uppercase tracking-tight">
                                <x-ui.icon name="euro-sign" class="h-3 w-3 inline mr-1" />
                                Budget/Gg
                            </th>
                            <th class="px-3 py-3 text-right text-xs font-semibold text-base-content uppercase tracking-tight">
                                <x-ui.icon name="wallet" class="h-3 w-3 inline mr-1" />
                                Sett.
                            </th>
                            <th class="px-3 py-3 text-right text-xs font-semibold text-base-content uppercase tracking-tight">
                                <x-ui.icon name="chart-line" class="h-3 w-3 inline mr-1" />
                                CPL
                            </th>
                            <th class="px-3 py-3 text-right text-xs font-semibold text-base-content uppercase tracking-tight">
                                <x-ui.icon name="user-check" class="h-3 w-3 inline mr-1" />
                                Lead/Gg
                            </th>
                            <th class="px-3 py-3 text-right text-xs font-semibold text-base-content uppercase tracking-tight">
                                <x-ui.icon name="users" class="h-3 w-3 inline mr-1" />
                                Lead Sett.
                            </th>
                        </tr>
                    </thead>
    ...
    <script>
        ...
        const giorniLavorativi = {{ $prospetto->giorni_lavorativi ?? 24 }};
        ...
        function renderMonthlyBudget() {
            ...
            // Calcola il budget mensile totale progressivo settimana per settimana
            // Formula: Somma (Budget giornaliero settimana × 6 giorni) per ogni settimana 1-4
            let monthlyBudget = 0;
            ...
            campaignData.accounts.forEach(account => {
                if (account.weeks && account.weeks.length > 0) {
                    ...
                    // Somma il budget di ogni settimana × 6 giorni lavorativi
                    effectiveWeeks.forEach(week => {
                        monthlyBudget += week.budget * 6;
                    });
                }
            });
            
            const avgDailyBudget = monthlyBudget / giorniLavorativi;
                    ...
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <div class="flex-1">
                            ...
                            <div class="flex items-center text-sm opacity-90">
                                ...
                                <span>Budget medio giornaliero: € ${avgDailyBudget.toFixed(2)} × ${giorniLavorativi} giorni lavorativi</span>
                            </div>
                        </div>
                        <div class="text-right">
        ...
        function renderMonthlyBudgetForAccount(account) {
            ...
            // Calcola il budget mensile progressivo settimana per settimana
            // Formula: Somma (Budget giornaliero settimana × 6 giorni) per ogni settimana 1-4
            const effectiveWeeks = account.weeks.filter(w => w.week >= 1);
            
            if (effectiveWeeks.length === 0) {
                container.innerHTML = '<div class="alert alert-warning">Nessuna settimana effettiva disponibile</div>';
                return;
            }
        ...
        function calculateTotalStats() {
            let monthlyBudget = 0; // Budget mensile = somma dei budget settimanali
            let totalCPL = 0;
            let cplCount = 0;
            let finalDailyBudget = 0; // Budget giornaliero finale (ultima settimana)
            ...
            campaignData.accounts.forEach(account => {
                if (account.weeks && account.weeks.length > 0) {
                    ...
                    if (effectiveWeeks.length > 0) {
                        // Budget mensile = somma diretta dei budget settimanali (budget × 6 giorni per settimana)
                        effectiveWeeks.forEach(week => {
                            monthlyBudget += week.budget * 6;
                        });
                        
                        // Budget finale = ultima settimana
                        const lastWeek = effectiveWeeks[effectiveWeeks.length - 1];
                        finalDailyBudget += lastWeek.budget;
                        
                        // CPL medio = somma tutti i CPL delle settimane 1-4 e fai la media
                        effectiveWeeks.forEach(week => {
                            totalCPL += week.cpl;
                            cplCount++;
                        });
            ...
            // Calcola CPL medio di tutte le 4 settimane
            const avgCPL = cplCount > 0 ? totalCPL / cplCount : 0;
            
            // Lead mensili = Budget mensile totale / CPL medio
            const monthlyLeads = avgCPL > 0 ? monthlyBudget / avgCPL : 0;
            const dailyLeads = monthlyLeads / giorniLavorativi;
            const avgDailyBudget = monthlyBudget / giorniLavorativi;
            
            return {
                budget: avgDailyBudget,
                finalBudget: finalDailyBudget,
                monthlyBudget: monthlyBudget, // Budget mensile totale
                leads: dailyLeads,
                cpl: avgCPL,
        ...
        function calculateAccountStats(account) {
            if (!account.weeks || account.weeks.length === 0) {
                return { budget: 0, finalBudget: 0, monthlyBudget: 0, leads: 0, cpl: 0, budgetGrowth: 0, leadsGrowth: 0, monthlyLeads: 0 };
            }
            ...
            // Budget finale = ultima settimana
            const lastWeek = effectiveWeeks[effectiveWeeks.length - 1];
            const finalDailyBudget = lastWeek.budget;
            
            // CPL medio = media di tutte le settimane effettive
            let totalCPL = 0;
            effectiveWeeks.forEach(week => {
                totalCPL += week.cpl;
            });
            const avgCPL = totalCPL / effectiveWeeks.length;
            ...
            // Calcola crescita tra prima e ultima settimana effettiva
            const firstWeek = effectiveWeeks[0];
                ...
                container.innerHTML = `
                    <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white card-hover">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="text-sm bg-blue-400 px-2 py-1 rounded">Medio</span>
                        </div>
                        <div class="text-3xl font-bold mb-1">€ ${stats.budget.toLocaleString('it-IT')}</div>
                        <div class="text-blue-100 text-sm">Budget Giornaliero Medio</div>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-xl shadow-lg p-6 text-white card-hover">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="text-sm bg-indigo-400 px-2 py-1 rounded">Finale</span>
                        </div>
                        <div class="text-3xl font-bold mb-1">€ ${stats.finalBudget.toLocaleString('it-IT')}</div>
                        <div class="text-indigo-100 text-sm">Budget Giornaliero Finale</div>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl shadow-lg p-6 text-white card-hover">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="text-sm bg-orange-400 px-2 py-1 rounded">Totale</span>
                        </div>
                        <div class="text-3xl font-bold mb-1">${Math.round(stats.monthlyLeads).toLocaleString('it-IT')}</div>
                        <div class="text-orange-100 text-sm">Lead Mese (${giorniLavorativi} gg)</div>
                    </div>
                `;
            } else {
                container.innerHTML = `
        ...
        function renderAccountTable(account) {
            ...
            account.weeks.forEach((week, index) => {
                ...
                const weeklyBudget = week.budget * 6; // Budget speso nella settimana (6 giorni)
                
                row.innerHTML = `
                    <td class="px-3 py-3 whitespace-nowrap">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs" style="background-color: ${account.color}20;">
                                <span class="font-bold" style="color: ${account.color};">${weekNumber}</span>
                            </div>
                            <span class="text-xs font-medium text-base-content hidden lg:inline">${weekLabel}</span>
                        </div>
                    </td>
                    <td class="px-3 py-3 whitespace-nowrap">
                        <span class="px-2 py-1 inline-flex text-xs leading-4 font-semibold rounded-full ${incrementClass}">
                            ${week.increment}
                        </span>
                    </td>
                    <td class="px-3 py-3 whitespace-nowrap text-right text-sm font-semibold text-base-content">
                        €${week.budget.toLocaleString('it-IT', {maximumFractionDigits: 0})}
                    </td>
                    <td class="px-3 py-3 whitespace-nowrap text-right text-sm font-bold text-info">
                        €${weeklyBudget.toLocaleString('it-IT', {maximumFractionDigits: 0})}
                    </td>
                    <td class="px-3 py-3 whitespace-nowrap text-right text-sm font-semibold text-base-content">
                        €${week.cpl.toFixed(1)}
                    </td>
                    <td class="px-3 py-3 whitespace-nowrap text-right text-sm font-semibold text-base-content">
                        ${week.leads.toFixed(1)}
                    </td>
                    <td class="px-3 py-3 whitespace-nowrap text-right text-sm font-bold text-success">
                        ${weeklyLeads.toLocaleString('it-IT')}
                    </td>
                `;
        ...
        function renderAggregatedTable() {
            ...
            for (let weekIndex = 0; weekIndex < maxWeeks; weekIndex++) {
                ...
                const weeklyBudget = totalBudget * 6; // Budget speso nella settimana (6 giorni)
                ...
                row.innerHTML = `
                    <td class="px-3 py-3 whitespace-nowrap">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 rounded-full bg-gradient-to-br from-purple-100 to-blue-100 dark:from-purple-900/30 dark:to-blue-900/30 flex items-center justify-center text-xs">
                    ...
                    <td class="px-3 py-3 whitespace-nowrap text-right text-sm font-bold text-base-content">
                        €${totalBudget.toLocaleString('it-IT', {maximumFractionDigits: 0})}
                    </td>
                    <td class="px-3 py-3 whitespace-nowrap text-right text-sm font-bold text-info">
                        €${weeklyBudget.toLocaleString('it-IT', {maximumFractionDigits: 0})}
                    </td>
