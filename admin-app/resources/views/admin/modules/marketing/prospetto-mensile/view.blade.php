<x-admin.wrapper :containerless="true">
    ...
    <script>
        ...
        function renderMonthlyBudget() {
            ...
            // Calcola il budget mensile totale usando l'ultima settimana effettiva (week >= 1)
            // Formula: Budget giornaliero finale × giorni lavorativi (6 giorni × 4 settimane = 24)
            ...
            const monthlyBudget = totalDailyBudget * giorniLavorativi;
                    ...
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <div class="flex-1">
                            ...
                            <div class="flex items-center text-sm opacity-90">
                                ...
                                <span>Budget giornaliero finale: € ${totalDailyBudget.toLocaleString('it-IT')} × ${giorniLavorativi} giorni lavorativi</span>
                            </div>
                        </div>
                        <div class="text-right">
        ...
        function renderMonthlyBudgetForAccount(account) {
            ...
            // Filtra solo le settimane >= 1 (esclude week 0)
            // Usa budget giornaliero finale × giorni lavorativi
            const effectiveWeeks = account.weeks.filter(w => w.week >= 1);
            let lastWeek;
            ...
            const monthlyBudget = lastWeek.budget * giorniLavorativi;
                ...
                <div class="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-2xl shadow-2xl p-8 text-white card-hover">
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <div class="flex-1">
                            ...
                            <div class="flex items-center text-sm opacity-90">
                                <i class="fas fa-info-circle mr-2"></i>
                                <span>Budget giornaliero finale: € ${lastWeek.budget.toLocaleString('it-IT')} × ${giorniLavorativi} giorni lavorativi</span>
                            </div>
                        </div>
                        <div class="text-right">
                            <div class="bg-white bg-opacity-20 rounded-xl p-4">
        ...
        function calculateTotalStats() {
            let totalBudgetFinal = 0; // Budget giornaliero finale (ultima settimana)
            let totalCPL = 0;
            let cplCount = 0;
            ...
            campaignData.accounts.forEach(account => {
                if (account.weeks && account.weeks.length > 0) {
                    ...
                    if (effectiveWeeks.length > 0) {
                        ...
                        const lastWeek = effectiveWeeks[effectiveWeeks.length - 1];
                        totalBudgetFinal += lastWeek.budget;
            ...
            // Calcola CPL medio di tutte le 4 settimane di novembre (media globale)
            const avgCPL = cplCount > 0 ? totalCPL / cplCount : 0;
            const monthlyBudget = totalBudgetFinal * giorniLavorativi;
            const monthlyLeads = avgCPL > 0 ? monthlyBudget / avgCPL : 0;
            const dailyLeads = monthlyLeads / giorniLavorativi;
            
            return {
                budget: totalBudgetFinal,
                leads: dailyLeads,
                cpl: avgCPL,
        ...
        function calculateAccountStats(account) {
            if (!account.weeks || account.weeks.length === 0) {
                return { budget: 0, leads: 0, cpl: 0, budgetGrowth: 0, leadsGrowth: 0, monthlyLeads: 0 };
            }
            ...
            // Budget finale = ultima settimana
            const lastWeek = effectiveWeeks[effectiveWeeks.length - 1];
            const finalBudget = lastWeek.budget;
            ...
            // Lead mensili = Budget mensile / CPL medio
            const monthlyBudget = finalBudget * giorniLavorativi;
            const monthlyLeads = avgCPL > 0 ? monthlyBudget / avgCPL : 0;
            const dailyLeads = monthlyLeads / giorniLavorativi;
                ...
                container.innerHTML = `
                    <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white card-hover">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="text-sm bg-blue-400 px-2 py-1 rounded">Finale</span>
                        </div>
                        <div class="text-3xl font-bold mb-1">€ ${stats.budget.toLocaleString('it-IT')}</div>
                        <div class="text-blue-100 text-sm">Budget Giornaliero Totale</div>
                    </div>
