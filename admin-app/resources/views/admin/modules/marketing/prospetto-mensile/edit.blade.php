<x-admin.wrapper>
    <x-slot name="title">{{ __('Modifica Prospetto Mensile') }}</x-slot>
    
    <x-admin.page-header 
        title="Modifica Prospetto Mensile" 
        subtitle="Aggiorna i dati JSON del prospetto mensile campagne"
        icon="edit"
        iconColor="warning"
    >
        <x-slot name="actions">
            <a href="{{ route('admin.marketing.prospetto_mensile.view', $prospetto->id) }}" class="btn btn-outline btn-info gap-2">
    ...
    <form method="POST" action="{{ route('admin.marketing.prospetto_mensile.update', $prospetto->id) }}" id="prospettoForm">
        @csrf
        @method('PUT')
                ...
                <x-admin.card tone="light" shadow="lg" padding="lg">
                    ...
                    <div class="form-control mb-4">
                        ...
                        <select name="mese" class="select select-bordered w-full @error('mese') select-error @enderror" required>
                            ...
                            @foreach($mesi as $numero => $nome)
                                <option value="{{ $numero }}" {{ old('mese', $prospetto->mese) == $numero ? 'selected' : '' }}>
                                    {{ $nome }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-control mb-4">
                        ...
                        <select name="anno" class="select select-bordered w-full @error('anno') select-error @enderror" required>
                            ...
                            @foreach($anni as $anno)
                                <option value="{{ $anno }}" {{ old('anno', $prospetto->anno) == $anno ? 'selected' : '' }}>
                                    {{ $anno }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-control mb-4">
                        ...
                        <input 
                            ...
                            value="{{ old('giorni_lavorativi', $prospetto->giorni_lavorativi) }}" 
                            min="1" 
                            max="31" 
                            placeholder="es: 24" 
                    ...
                    <div class="form-control mb-4">
                        ...
                        >{{ old('descrizione', $prospetto->descrizione) }}</textarea>
                        @error('descrizione')
                            <label class="label">
                                <span class="label-text-alt text-error">{{ $message }}</span>
                            </label>
                ...
                <x-admin.card tone="light" shadow="lg" padding="lg">
                    ...
                    <div class="form-control mb-4">
                        ...
                        <textarea 
                            ...
                            required
                        >{{ old('dati_json', json_encode($prospetto->dati_accounts, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE)) }}</textarea>
                        @error('dati_json')
                            <label class="label">
                                <span class="label-text-alt text-error">{{ $message }}</span>
                            </label>
